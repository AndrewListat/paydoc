...
use app\modules\ls_admin\models\Category;
...
class SlugCat extends Behavior
{
    ...
    {
        $cat=Category::find()->where(['fid' => $this->owner->{$this->cat_id}])
            ->one();
        if ($cat){
            $url = $cat->furl.'/';
        } else {
        ...
        if ( empty( $this->owner->{$this->out_attribute} ) ) {
            $this->owner->{$this->out_attribute} = $url.$this->generateSlug( $this->owner->{$this->in_attribute} );
        } else {
            $this->owner->{$this->out_attribute} = $url.$this->generateSlug( $this->owner->{$this->out_attribute} );
        }
    }
